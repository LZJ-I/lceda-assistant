...
pub fn export(client: &LcedaClient, item: &SearchItem, req: &ExportRequest) -> Result<DownloadPaths> {
    ...
    if req.step {
        ...
        let path = req.out_dir.join(format!("{base}.step"));
    ...
    if req.obj {
        ...
        let obj_path = req.out_dir.join(format!("{base}.obj"));
        let mtl_path = req.out_dir.join(format!("{base}.mtl"));
    ...
    if req.datasheet {
        if let Some(url) = item.datasheet_url() {
            let path = req.out_dir.join(format!("{base}.pdf"));
            if req.force || !path.exists() {
                ...
                if bytes.starts_with(b"%PDF") {
                    ...
                    eprintln!("数据手册不是 PDF，已跳过");
                }
            } else {
                out.datasheet = Some(path);
            }
        } else {
            eprintln!("该器件没有数据手册链接");
        }
    }
    ...
    if req.any_library() {
        ...
        let (symbol_json, footprint_json, symbol_ir, footprint_ir) =
            fetch_sources(client, item, &req.out_dir, &base, req.force)?;
        ...
        if req.ad {
            if let Err(e) = export_altium(&mut out, &req.out_dir, &base, symbol_ir.as_ref(), footprint_ir.as_ref())
            {
                if !req.kicad {
                    return Err(e);
                }
